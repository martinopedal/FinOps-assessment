#!/usr/bin/env python3
"""Generate PowerShell full-document HTML conformance golden (Phase 5c)."""
...
from finops_assess.reporters.html_reporter import build_html_report  # noqa: E402
...
HTML_GOLDEN = FIXTURE_DIR / "demo-report.html"
...
def regenerate() -> dict[Path, str]:
    ...
    try:
        ...
        html = build_html_report(source_report)
    finally:
...
def _check() -> int:
    ...
    if stale:
        for path in stale:
            ...
            print(f"stale: {rel}; run python scripts/generate_ps_html_fixtures.py")
        return 1
    print("ok: html fixture is up-to-date")
    return 0
