...
- **PowerShell engine — GitHub + ADO rules (Phase 4).**
  ...
  test in `Invoke-FinOpsAssessment.Tests.ps1` is updated to reflect that
  12 `AZ.*` rules remain skipped (not 20) and that `github`/`ado`
  surfaces are now produced. **Honest scope:** GitHub + ADO rule slice +
  CSV parity only. The 12 `AZ.*` rules remain in
  `rules_skipped_no_impl` and are deferred to Phase 3.
